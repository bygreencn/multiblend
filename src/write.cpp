...
void tiff_out() {
    ...
    TIFFSetField(g_tiff, TIFFTAG_COMPRESSION, (uint16)g_compression);
    ...
    output(1, "tiffout: with mask, 1%d images \n", g_numimages);

    for (s=0; s<strips; s++) 
    {
        output(1, "tiffout: with mask, strips: 1%d of 2%d strips\n", s, strips);
        ...
        output(1, "tiffout: strip 1%d write \n", s);
        TIFFWriteEncodedStrip(g_tiff, s, strip, rows*mul);

        remaining-=rows;
    }// for loop, every strip
